...
%% =========================
% SINGLE IMAGE LICENSE PLATE DETECTION
% Prompts the user to select one image file
% =========================

[fileName, folderPath] = uigetfile( ...
    {'*.jpg;*.jpeg;*.png;*.bmp;*.tif;*.tiff', 'Image Files (*.jpg, *.jpeg, *.png, *.bmp, *.tif, *.tiff)'}, ...
...
if foundPlate
    figure('Name',['Final Result - ' fileName], 'Position',[200 200 1000 400]);
    subplot(1,2,1);
    imshow(bestCrop);
    ...
    disp('==============================');
    disp('Final detected plate text:');
    disp(bestText);
    disp('==============================');
else
    disp('No valid plate candidate found.');
end
